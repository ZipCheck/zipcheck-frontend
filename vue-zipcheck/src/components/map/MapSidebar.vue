<template>
	<aside
		class="w-96 bg-surface-light dark:bg-surface-dark border-r border-gray-200 dark:border-gray-800 flex flex-col z-20 shadow-xl absolute md:relative h-full transform -translate-x-full md:translate-x-0 transition-transform duration-300"
	>
		<div class="p-5 border-b border-gray-100 dark:border-gray-800">
			<h2 class="text-lg font-bold mb-3">어떤 동네를 찾으시나요?</h2>
			<!-- 시/도, 구/군, 동 드롭다운 추가 -->
			<div class="space-y-2 mb-4">
				<select :value="selectedSido" @change="$emit('update:selectedSido', $event.target.value)"
				...
				<select :value="selectedGugun" @change="$emit('update:selectedGugun', $event.target.value)" :disabled="!selectedSido"
					class="w-full bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-sm py-2 rounded-xl focus:ring-2 focus:ring-primary focus:border-transparent outline-none">
					<option value="">구/군 선택</option>
					<option v-for="gugun in gugunList" :key="gugun" :value="gugun">{{ gugun }}</option>
				</select>
				<select :value="selectedDong" @change="$emit('update:selectedDong', $event.target.value)" :disabled="!selectedGugun"
					class="w-full bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-sm py-2 rounded-xl focus:ring-2 focus:ring-primary focus:border-transparent outline-none">
		...
		<div
			class="px-5 py-3 border-b border-gray-100 dark:border-gray-800 flex gap-2 overflow-x-auto no-scrollbar"
		>
			<button
			...
			>
				<span class="material-symbols-outlined text-[16px]">tune</span> 필터
			</button>
		...
		<div v-if="showFilterPanel" class="p-5 border-b border-gray-100 dark:border-gray-800 space-y-4">
            <h3 class="font-bold text-md">가격 범위 (만원)</h3>
            <div class="flex items-center gap-2">
                <input type="number" :value="minPrice" @input="$emit('update:minPrice', $event.target.value ? Number($event.target.value) : null)" placeholder="최소 가격"
                ...
                <span>~</span>
                <input type="number" :value="maxPrice" @input="$emit('update:maxPrice', $event.target.value ? Number($event.target.value) : null)" placeholder="최대 가격"
                    class="w-1/2 bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-sm py-2 rounded-xl focus:ring-2 focus:ring-primary focus:border-transparent outline-none">
            </div>

            <h3 class="font-bold text-md">면적 범위 (m²)</h3>
            <div class="flex items-center gap-2">
                <input type="number" :value="minArea" @input="$emit('update:minArea', $event.target.value ? Number($event.target.value) : null)" placeholder="최소 면적"
                    class="w-1/2 bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-sm py-2 rounded-xl focus:ring-2 focus:ring-primary focus:border-transparent outline-none">
                <span>~</span>
                <input type="number" :value="maxArea" @input="$emit('update:maxArea', $event.target.value ? Number($event.target.value) : null)" placeholder="최대 면적"
                    class="w-1/2 bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-sm py-2 rounded-xl focus:ring-2 focus:ring-primary focus:border-transparent outline-none">
            </div>
            <button @click="$emit('search-properties')"
		...
		<div
			class="flex-grow overflow-y-auto p-4 space-y-4 bg-background-light dark:bg-background-dark"
		>
            <!-- 선택된 매물이 있을 때 "전체 목록 보기" 버튼 표시 -->
            <div v-if="selectedPropertyId" class="flex justify-between items-center bg-blue-50 dark:bg-blue-900/20 p-3 rounded-xl border border-blue-100 dark:border-blue-800">
                <span class="text-sm font-bold text-blue-700 dark:text-blue-300">선택된 매물</span>
                <button @click="$emit('reset-selection')" class="text-xs text-gray-500 hover:text-gray-800 underline">
                    전체 목록 보기
                </button>
            </div>

			<div v-if="loading" class="text-center py-4">매물 정보를 불러오는 중...</div>
			<div v-else-if="error" class="text-center py-4 text-red-500">
				매물 정보를 불러오는데 실패했습니다: {{ error.message }}
			</div>
			<div v-else-if="filteredProperties.length === 0" class="text-center py-4 text-gray-500">
				검색 결과가 없습니다.
			</div>
			<!-- 클러스터 목록 (줌 아웃 시) -->
			<template v-if="properties.length > 0 && properties[0].regionName !== undefined">
				<div 
				...
				>
					<span class="font-bold text-gray-800 dark:text-gray-200">{{ cluster.regionName }}</span>
					<span class="bg-primary/10 text-primary px-3 py-1 rounded-full text-sm font-bold">{{ cluster.dealCount }}개</span>
				</div>
			</template>

            <!-- 아파트 목록 (줌 인 시) -->
            <template v-else-if="properties.length > 0 && properties[0].aptSeq !== undefined">
                <div 
                ...
                >
                    <div class="flex justify-between items-center mb-1">
                        <span class="font-bold text-gray-800 dark:text-gray-200">{{ apt.aptName }}</span>
                        <span class="bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 text-xs px-2 py-0.5 rounded-full font-bold">
                            {{ apt.dealCount }}건
                        </span>
                    </div>
                </div>
			...
			<template v-else>
				<PropertyCard
					...
					:property="property"
					:class="{'ring-2 ring-primary bg-primary/5': selectedPropertyId === (property.deal_id || property.no)}"
				/>
			</template>
		</div>

        <!-- 페이지네이션 -->
        <div v-if="pagingInfo" class="p-4 border-t border-gray-100 dark:border-gray-800 flex justify-center items-center gap-2 bg-white dark:bg-gray-900 shrink-0 z-30 relative shadow-inner">
            <button 
